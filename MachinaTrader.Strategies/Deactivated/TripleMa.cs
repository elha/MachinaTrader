        ...
        public override List<TradeAdvice> Prepare(List<Candle> candles)
        {
            ...
            for (int i = 0; i < candles.Count; i++)
            {
                if (i == 0)
                    ...
                    result.Add(TradeAdvice.Factory.Buy); // A cross of the EMA and long SMA is a buy signal.
                else if ((ema11[i] < sma50[i] && ema11[i - 1] > sma50[i - 1]) || (ema11[i] < sma20[i] && ema11[i - 1] > sma20[i - 1]))
                    result.Add(TradeAdvice.Factory.Sell); // As soon as our EMA crosses below an SMA its a sell signal.
                else
                    result.Add(TradeAdvice.Factory.Hold);
            }
